 ...
 */
const imageminTest = done => {
  const imagemin = require('../test-make-common/external-modules/gulp-imagemin').default
  const imagesFile = `${srcPath}/imageToCopy.svg`
  writeFileSync(imagesFile, imageContents)
  return src(imagesFile)
    .pipe(
      imagemin(
        {
          interlaced: true,
    ...
    .on('finish', () => {
      ...
      const movedContents = readFileSync(`${browserPath}/imageToCopy.svg`).toString()
...
describe('makeCommon', () => {
  test('converts the source files into common js', (done) => {
    ...
    makeCommon(srcFile, `${vendorPath}/gulp-imagemin`, {
      ...
      .on('finish', () => {
        ...
        setTimeout(imageminTest, 800, done)
      })
      .on('error', error => {
        console.error('Encountered error', error)
        done()
